...
# HVD-3 Sharding the data
dataset = dataset.shard(num_shards=hvd.size(), index=hvd.rank())
#Do we get better results if we don't shard the test data? Yes, at least for n=2
...
opt = tf.optimizers.Adam(0.01*hvd.size()) # tf.optimizers.Adam(args.lr)
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        total_loss = hvd.allreduce(loss_value, average=True) #changed from parameter being "loss" to "loss_value"
        total_acc = hvd.allreduce(acc, average=True)
        loss_value = total_loss
        acc = total_acc
        # HVD - 6 - Broadcast model and parameters from rank 0 to the other ranks
        # prev ep==0 but Thomas on the forum showed what's below
        if (nstep==0 and ep==0 and hvd.rank()==0): #previously had step_in_epoch and epoch but those are in the resnet code
            hvd.broadcast_variables(mnist_model.variables, root_rank=0)
            hvd.broadcast_variables(opt.variables(), root_rank=0)
        #Not sure if I should delete the two lines below...
        training_loss += loss_value/nstep
        training_acc += acc/nstep #definitely keep "batch % 100 == 0" below
...
#Commenting this out got the time statements to output properly: checkpoint.save(checkpoint_dir)
